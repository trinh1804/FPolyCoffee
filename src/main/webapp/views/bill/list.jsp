    ...
    <%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
        <%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
            <%@ include file="/views/layout/header.jsp" %>
                ...
                <!-- Tổng tất cả hóa đơn -->
                <c:set var="totalAll" value="${waitingCount + finishCount + cancelCount}" />

                <!-- Status filter tabs -->
                <div class="flex flex-wrap gap-2 mb-6">

                    <!-- Quan trọng: dùng status=-1 để servlet nhận đúng, tránh lỗi ParamUtil trả về 0 khi không có param -->
              ...
              ${statusFilter == -1 ? 'bg-primary text-white shadow-md' : 'bg-white text-on-surface-variant border border-outline-variant hover:border-primary hover:text-primary'}">
                     ...
                     ${statusFilter == -1 ? 'bg-white/25 text-white' : 'bg-surface-container text-on-surface-variant'}">
                            ${totalAll}
                        </span>
                    </a>
              ...
              ${statusFilter == 0 ? 'bg-amber-600 text-white shadow-md' : 'bg-white text-on-surface-variant border border-outline-variant hover:border-amber-500 hover:text-amber-700'}">
                        ...
                        Chờ xử lý
                        <span class="inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1 rounded-full text-[11px] font-bold
                     ${statusFilter == 0 ? 'bg-white/25 text-white' : 'bg-amber-50 text-amber-700'}">
                            ${waitingCount}
                        </span>
                    </a>
              ...
              ${statusFilter == 1 ? 'bg-emerald-600 text-white shadow-md' : 'bg-white text-on-surface-variant border border-outline-variant hover:border-emerald-500 hover:text-emerald-700'}">
                        ...
                        Hoàn thành
                        <span class="inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1 rounded-full text-[11px] font-bold
                     ${statusFilter == 1 ? 'bg-white/25 text-white' : 'bg-emerald-50 text-emerald-700'}">
                            ${finishCount}
                        </span>
                    </a>
              ...
              ${statusFilter == 2 ? 'bg-red-600 text-white shadow-md' : 'bg-white text-on-surface-variant border border-outline-variant hover:border-red-400 hover:text-red-600'}">
                        ...
                        Đã hủy
                        <span class="inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1 rounded-full text-[11px] font-bold
                     ${statusFilter == 2 ? 'bg-white/25 text-white' : 'bg-red-50 text-red-600'}">
                            ${cancelCount}
                        </span>
                    </a>
                </div>
                                            ...
                                            <c:choose>
                                                <c:when test="${bill.status == 0}">
                                                    <span class="badge badge-warning">
                                                        <span
                                                            class="material-symbols-outlined text-[13px]">pending</span>Chờ
                                                        xử lý
                                                    </span>
                                                </c:when>
                                                <c:when test="${bill.status == 1}">
                                                    <span class="badge badge-success">
                                                        <span
                                                            class="material-symbols-outlined text-[13px]">check_circle</span>Hoàn
                                                        thành
                                                    </span>
                                                </c:when>
                                                <c:when test="${bill.status == 2}">
                                                    <span class="badge badge-danger">
                                                        <span
                                                            class="material-symbols-outlined text-[13px]">cancel</span>Đã
                                                        hủy
                                                    </span>
                                                </c:when>
                                            </c:choose>
                                ...
                                <c:if test="${empty bills}">
                                    <tr>
                                        <td colspan="8" class="text-center py-16 text-on-surface-variant">
                                            ...
                                            <p class="text-sm opacity-60">Thử chọn tab khác để xem</p>
                                        </td>
                                    </tr>
                                </c:if>
                            </tbody>
                    ...
                    <c:if test="${totalPages > 1}">
                        <div class="px-5 py-4 flex items-center justify-between border-t border-outline-variant/30">
                            <p class="text-sm text-on-surface-variant">
                                Trang <strong>${currentPage}</strong> / ${totalPages} &nbsp;·&nbsp; ${totalRecords}
                                hóa đơn
                            </p>
