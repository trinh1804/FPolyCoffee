...
<div class="-mx-6 -mt-8 relative overflow-hidden"
     style="background: linear-gradient(135deg, #3d1f0d 0%, #682d00 50%, #874210 100%); min-height: 440px;">
    <div class="absolute inset-0 opacity-10"
         style="background-image: radial-gradient(circle at 70% 50%, #ffb68c 0%, transparent 60%),
                                  radial-gradient(circle at 20% 80%, #ffdbc9 0%, transparent 40%);">
    </div>
    <div class="relative max-w-screen-xl mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
        <div>
            <span class="inline-block py-1 px-3 bg-white/15 text-primary-fixed text-xs font-bold rounded-full mb-4 uppercase tracking-widest">
                Premium Brewing
            </span>
            <h1 class="font-headline text-5xl font-black text-white mb-4 leading-tight">
                Savour the<br>Rich Aroma &<br>Bold Flavor
            </h1>
            <p class="text-primary-fixed-dim text-lg mb-6 leading-relaxed max-w-md">
                ...
                <c:if test="${sessionScope.user != null}">
                    ...
                    <c:if test="${not empty openBills}">
                        <a href="${pageContext.request.contextPath}/employee/bills"
                           class="btn btn-outline text-base px-6 py-3"
            ...
            <div class="flex gap-8 mt-8">
                <div>
                    <div class="font-headline font-black text-3xl text-white">${drinks.size()}+</div>
        ...
        <div class="hidden lg:flex items-center justify-center">
            <div class="w-64 h-64 rounded-[2rem] bg-white/10 backdrop-blur-sm border border-white/20
                        flex items-center justify-center rotate-6">
                <span class="material-symbols-outlined text-white/60"
                      style="font-size:9rem;font-variation-settings:'FILL' 1,'wght' 300">local_cafe</span>
            </div>
        </div>
    </div>
</div>
...
<div class="mt-14">
    <div class="text-center mb-8">
        <span class="inline-block py-1 px-3 bg-primary-fixed text-primary text-xs font-bold rounded-full mb-3 uppercase tracking-widest">
            Thực đơn
        </span>
        <h2 class="font-headline font-extrabold text-3xl text-primary">Đồ Uống Của Chúng Tôi</h2>
    ...
    <div class="flex flex-wrap justify-center gap-2 mb-8" id="catTabs">
        <button onclick="filterMenu('all')" data-cat="all"
                class="menu-cat-btn px-5 py-2 rounded-full text-sm font-semibold transition-all
                       bg-primary text-white shadow">
            Tất cả
        </button>
        <c:forEach items="${categories}" var="cat">
            <button onclick="filterMenu('${cat.id}')" data-cat="${cat.id}"
                    class="menu-cat-btn px-5 py-2 rounded-full text-sm font-semibold transition-all
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4" id="menuGrid">
        <c:forEach items="${drinks}" var="drink">
            <div class="menu-item" data-cat="${drink.categoryId}">
                <div class="pc-card overflow-hidden group hover:-translate-y-1 transition-transform duration-200">
                    <%-- Image --%>
        ...
        <c:if test="${empty drinks}">
            <div class="col-span-5 text-center py-20 text-on-surface-variant">
                <span class="material-symbols-outlined block mb-3 opacity-25" style="font-size:5rem">coffee_off</span>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="pc-card p-8 group hover:-translate-y-2 transition-transform duration-300">
            <div class="w-14 h-14 rounded-2xl bg-primary-fixed flex items-center justify-center mb-6
                ...
                <span class="material-symbols-outlined text-primary group-hover:text-white text-2xl">coffee</span>
            </div>
        ...
        <div class="pc-card p-8 group hover:-translate-y-2 transition-transform duration-300">
            <div class="w-14 h-14 rounded-2xl bg-primary-fixed flex items-center justify-center mb-6
                        group-hover:bg-primary group-hover:text-white transition-colors">
                <span class="material-symbols-outlined text-primary group-hover:text-white text-2xl">verified</span>
            </div>
            <h3 class="font-headline font-bold text-xl text-primary mb-3">Chất lượng hàng đầu</h3>
            <p class="text-on-surface-variant text-sm leading-relaxed">
                Hạt cà phê chất lượng từ các vùng cao nguyên nổi tiếng, rang xay tại chỗ mỗi ngày.
            </p>
        </div>
        <div class="pc-card p-8 group hover:-translate-y-2 transition-transform duration-300">
            <div class="w-14 h-14 rounded-2xl bg-primary-fixed flex items-center justify-center mb-6
                        group-hover:bg-primary group-hover:text-white transition-colors">
                <span class="material-symbols-outlined text-primary group-hover:text-white text-2xl">weekend</span>
            </div>
