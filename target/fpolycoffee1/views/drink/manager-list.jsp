        ...
        <form action="${pageContext.request.contextPath}/manager/drinks" method="get">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                    ...
                    <input type="text" class="form-control" name="searchName" value="${searchName}" placeholder="Nhập tên">
                </div>
                <div>
        ...
        <table class="data-table w-full">
            ...
            <tbody>
                <c:forEach items="${drinks}" var="drink">
                    <tr>
                        <td class="text-center" style="color:#9E7B5E;font-size:0.85rem">${drink.id}</td>
                            ...
                            <c:choose>
                                <c:when test="${not empty drink.image}">
                                    ...
                                    <div class="rounded-xl mx-auto hidden items-center justify-center"
                                        ...
                                        <span class="material-symbols-outlined" style="color:#C6956A;font-size:20px">coffee</span>
                                    </div>
                                </c:when>
                                <c:otherwise>
                            ...
                            <c:choose>
                                <c:when test="${drink.active}">
                                    <span class="badge badge-success">
                                        <span class="material-symbols-outlined text-[12px]">check_circle</span> Đang bán
                                    </span>
                                </c:when>
                                <c:otherwise>
                                    <span class="badge badge-neutral">
                                        <span class="material-symbols-outlined text-[12px]">block</span> Ngừng bán
                                    </span>
                                </c:otherwise>
                            </c:choose>
                        </td>
                        <td class="text-center">
                            <div class="flex items-center justify-center gap-1.5">
                                <%-- Edit --%>
                                <button onclick="openEditModal(${drink.id},'${drink.name.replace("'","\\'")}',${drink.price},${drink.categoryId},'${drink.description.replace("'","\\'").replace('"','&quot;')}',${drink.active})"
                                        class="btn btn-secondary btn-icon" title="Sửa">
                                    <span class="material-symbols-outlined text-[16px]">edit</span>
                                </button>
                                <%-- Toggle mở/ẩn --%>
                                <form action="${pageContext.request.contextPath}/manager/drinks/toggle-status"
                ...
                <c:if test="${empty drinks}">
                    <tr>
                        <td colspan="7" class="text-center" style="padding:4rem;color:#9E7B5E">
                            <span class="material-symbols-outlined block mb-2" style="font-size:4rem;opacity:.25">coffee</span>
...
<div id="addModal" class="modal-backdrop" style="display:none" onclick="if(event.target===this)closeModal('addModal')">
    <div class="modal-box">
        <div class="modal-header">
            <span style="font-family:'Playfair Display',serif;font-weight:700;color:#F5E6D3">Thêm đồ uống mới</span>
        ...
        <div class="p-6">
            <form action="${pageContext.request.contextPath}/manager/drinks/add" method="post" enctype="multipart/form-data">
                <div class="mb-4">
                    <label class="form-label">Tên đồ uống <span style="color:#C62828">*</span></label>
                    <input type="text" class="form-control" name="name" placeholder="VD: Cà phê muối" required>
                </div>
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="form-label">Giá (VNĐ) <span style="color:#C62828">*</span></label>
                        <input type="number" class="form-control" name="price" step="1000" placeholder="35000" required min="1000">
                    </div>
                    <div>
                ...
                <div class="flex items-center gap-2 mb-5">
                    <input type="checkbox" id="addActive" name="active" value="true" checked class="accent-caramel w-4 h-4">
                    <label for="addActive" class="form-label mb-0 cursor-pointer">Đang bán</label>
                </div>
                <div class="flex gap-3">
                    <button type="submit" class="btn btn-primary flex-1 py-3">
                        <span class="material-symbols-outlined text-[18px]">add</span> Thêm
                    </button>
                    <button type="button" onclick="closeModal('addModal')" class="btn btn-outline flex-1 py-3">Hủy</button>
                </div>
            </form>
        </div>
    </div>
...
<div id="editModal" class="modal-backdrop" style="display:none" onclick="if(event.target===this)closeModal('editModal')">
    <div class="modal-box">
        <div class="modal-header">
            <span style="font-family:'Playfair Display',serif;font-weight:700;color:#F5E6D3">Chỉnh sửa đồ uống</span>
        ...
        <div class="p-6">
            <form action="${pageContext.request.contextPath}/manager/drinks/edit" method="post" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="form-label">Giá (VNĐ) <span style="color:#C62828">*</span></label>
                        <input type="number" class="form-control" id="editDrinkPrice" name="price" step="1000" min="1000" required>
                    </div>
                    <div>
                        <label class="form-label">Danh mục <span style="color:#C62828">*</span></label>
                ...
                <div class="mb-4">
                    <label class="form-label">Mô tả</label>
                    <textarea class="form-control" id="editDrinkDesc" name="description" rows="2"></textarea>
                </div>
                <div class="mb-4">
                    <label class="form-label">Hình ảnh mới <span style="font-size:0.78rem;color:#9E7B5E">(để trống giữ ảnh cũ)</span></label>
                    <input type="file" class="form-control" name="image" accept="image/*">
                </div>
                <div class="flex items-center gap-2 mb-5">
                    <input type="checkbox" id="editDrinkActive" name="active" value="true" class="accent-caramel w-4 h-4">
                ...
                <div class="flex gap-3">
                    ...
                    <button type="button" onclick="closeModal('editModal')" class="btn btn-outline flex-1 py-3">Hủy</button>
                </div>
            </form>
        </div>
    </div>
